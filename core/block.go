...
type BlockHeader struct {
	...
	RametronenterpriseVotes *AggregatedRametronenterpriseVotes `rlp:"nil"` // Added in Pando3.0 fork.
	TxHash             common.Hash
...
func (h *BlockHeader) EncodeRLP(w io.Writer) error {
	...
	if h.Height < common.HeightEnablePando1 {
		return rlp.Encode(w, []interface{}{
			h.ChainID,
			h.Epoch,
			h.Height,
	...
	// Pando2.0 fork
	if h.Height >= common.HeightEnablePando1 && h.Height < common.HeightEnablePando2 {
		return rlp.Encode(w, []interface{}{
			h.ChainID,
			h.Epoch,
			h.Height,
...
func (h *BlockHeader) DecodeRLP(stream *rlp.Stream) error {
	...
	if h.Height >= common.HeightEnablePando2 {
		...
		if common.Bytes2Hex(raw) == "c0" {
			h.RametronenterpriseVotes = nil
		} else {
			evotes := &AggregatedRametronenterpriseVotes{}
			rlp.DecodeBytes(raw, evotes)
			if err != nil {
				return err
			}
			h.RametronenterpriseVotes = evotes
		}
	}
...
func (h *BlockHeader) Validate(chainID string) result.Result {
	...
	if !h.Signature.Verify(h.SignBytes(), h.Proposer) {
		return result.Error("Signature verification failed block")
	}
	return result.OK
}
