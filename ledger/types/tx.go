...
type DepositStakeTxV2 struct {
	...
	Source  TxInput  `json:"source"`  // source staker account
	Holder  TxOutput `json:"holder"`  // stake holder account
	Purpose uint8    `json:"purpose"` // purpose e.g. stake for validator/guardian/rametronenterprise
...
type WithdrawStakeTx struct {
	...
	Source  TxInput  `json:"source"`  // source staker account
	Holder  TxOutput `json:"holder"`  // stake holder account
	Purpose uint8    `json:"purpose"` // purpose e.g. stake for validator/guardian/rametronenterprise node
}
...
//
// StakeRewardDistributionTx needs to be signed and submitted by the "stake holders", i.e. a guardian or an rametronenterprise node.
...
//
// The purpose of this transaction is to allow guardian/rametronenterprise node operators to charge a fee for the hosting service.
// The service fee (i.e. split fraction) can be specified by the guardian/rametronenterprise node operators via the SplitBasisPoint parameter.
...
// a free market for guardian/rametronenterprise node hosting service.
//
type StakeRewardDistributionTx struct {
	Fee             Coins    `json:"fee"`               // transction fee, NOT the hosting service fee
	Holder          TxInput  `json:"holder"`            // stake holder account, i.e., a guardian or an rametronenterprise node
	...
	//Purpose         uint8    `json:"purpose"`           // purpose e.g. stake for guardian/rametronenterprise node
}
...
const CHAIN_ID_OFFSET int64 = 3600
...
func mapChainIDWithoutOffset(chainIDStr string) *big.Int {
	if chainIDStr == "pandonet" { // correspond to the  mainnet
		return big.NewInt(1)
	} else if chainIDStr == "testnet" { // correspond to testnet
		return big.NewInt(2)
	} else if chainIDStr == "privatenet" { // correspond to privatenet
		return big.NewInt(3)
	}
