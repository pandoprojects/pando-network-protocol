...
import (
	...
	"github.com/pandoprojects/pando/store/treestore"
)
...
// GetRametronenterpriseStakeReturns gets the rametronenterprise stake returns
func (sv *StoreView) GetRametronenterpriseStakeReturns(height uint64) []StakeWithHolder {
	data := sv.Get(RametronenterpriseStakeReturnsKey(height))
	if data == nil || len(data) == 0 {
	...
	if err != nil {
		log.Panicf("Error reading rametronenterprise stake returns %v, error: %v",
			data, err.Error())
	}
	return returnedStakes
}

// GetRametronenterpriseStakeReturns saves the rametronenterprise stake returns for the given height
func (sv *StoreView) SetRametronenterpriseStakeReturns(height uint64, stakeReturns []StakeWithHolder) {
	returnedStakesBytes, err := types.ToBytes(stakeReturns)
	if err != nil {
		log.Panicf("Error writing rametronenterprise stake returns %v, error: %v",
			stakeReturns, err)
	}
	sv.Set(RametronenterpriseStakeReturnsKey(height), returnedStakesBytes)
}

// RemoveRametronenterpriseStakeReturns removes the rametronenterprise stake returns for the given height
func (sv *StoreView) RemoveRametronenterpriseStakeReturns(height uint64) {
	sv.Delete(RametronenterpriseStakeReturnsKey(height))
}

// GetTotalRametronenterpriseStake retrives the total active Rametronenterprise stakes
func (sv *StoreView) GetTotalRametronenterpriseStake() *big.Int {
	raw := sv.Get(RametronenterprisesTotalActiveStakeKey())
	return new(big.Int).SetBytes(raw)
}

// SetTotalRametronenterpriseStake sets the total active Rametronenterprise stakes
func (sv *StoreView) SetTotalRametronenterpriseStake(amount *big.Int) {
	sv.Set(RametronenterprisesTotalActiveStakeKey(), amount.Bytes())
}
