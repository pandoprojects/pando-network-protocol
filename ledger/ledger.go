...
import (
	...
	exec "github.com/pandoprojects/pando/ledger/execution"
...
// GetRametronenterprisePoolOfLastCheckpoint returns the rametronenterprise pool of the given block.
func (ledger *Ledger) GetRametronenterprisePoolOfLastCheckpoint(blockHash common.Hash) (core.RametronenterprisePool, error) {
	db := ledger.state.DB()
	store := kvstore.NewKVStore(db)

	// Find last checkpoint and retrieve RametronenterpriseP.
	block, err := findBlock(store, blockHash)
	if err != nil {
		return nil, err
	}
	blockHash = block.Hash()
	for {
		logger.Debugf("Ledger.GetRametronenterprisePoolOfLastCheckpoint, block.height = %v", block.Height)
		...
		if common.IsCheckPointHeight(block.Height) {
			stateRoot := block.BlockHeader.StateHash
			storeView := st.NewStoreView(block.Height, stateRoot, db)
			rametronenterprisep := state.NewRametronenterprisePool(storeView, true)
			return rametronenterprisep, nil
		}
		blockHash = block.Parent
	}
}
...
func (ledger *Ledger) handleDelayedStateUpdates(view *st.StoreView) {
	...
	if blockHeight >= common.HeightEnablePando2 {
		ledger.handleRametronenterpriseStakeReturns(view)
	}
}
...
func (ledger *Ledger) handleRametronenterpriseStakeReturns(view *st.StoreView) {
	currentHeight := view.Height()
	returnedStakesWithHolders := view.GetRametronenterpriseStakeReturns(currentHeight)
	if len(returnedStakesWithHolders) == 0 {
		return // no need to call view.RemoveRametronenterpriseStakeReturns()
	}
	...
	for _, returnedStakeWithHolder := range returnedStakesWithHolders {
		...
		rametronenterpriseAddress := returnedStakeWithHolder.Holder
		if !returnedStake.Withdrawn || currentHeight < returnedStake.ReturnHeight {
			log.Panicf("Cannot return stake: withdrawn = %v, returnHeight = %v, currentHeight = %v",
				returnedStake.Withdrawn, returnedStake.ReturnHeight, currentHeight)
		}
		...
		returnedCoins := types.Coins{ // Important: Rametronenterprise deposit/withdraw PTX stake, NOT Pando
			PandoWei: types.Zero,
			PTXWei: returnedStake.Amount,
		}
		sourceAccount.Balance = sourceAccount.Balance.Plus(returnedCoins)
		...
		// TODO: potentially O(m*n) runtime complexity, but the number of stakes on an rametronenterprise is bounded
		err := rametronenterprisep.ReturnStake(currentHeight, rametronenterpriseAddress, returnedStake)
		if err != nil {
			log.Panicf("Failed to return stake: currentHeight = %v, rametronenterpriseAddress = %v, returnedStake = %v, err = %v",
				currentHeight, rametronenterpriseAddress, returnedStake, err)
		}

		logger.Infof("Stake returned: rametronenterpriseAddress = %v, source = %v, amount = %v",
			rametronenterpriseAddress, returnedStake.Source, returnedStake.Amount)
	}

	view.RemoveRametronenterpriseStakeReturns(currentHeight)
}
...
func (ledger *Ledger) addCoinbaseTx(view *st.StoreView, proposer *core.Validator,
	...
	if guardianVotes != nil && ch >= common.HeightEnablePando1 && common.IsCheckPointHeight(ch) {
		guardianPool, rametronenterprisePool := exec.RetrievePools(ledger, ledger.chain, ledger.db, ch, guardianVotes, rametronenterpriseVotes)
		accountRewardMap = exec.CalculateReward(ledger, view, validatorSet, guardianVotes, guardianPool, rametronenterpriseVotes, rametronenterprisePool)
