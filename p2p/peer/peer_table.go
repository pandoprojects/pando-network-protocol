...
import (
	...
	mm "github.com/pandoprojects/pando/common/math"
	nu "github.com/pandoprojects/pando/p2p/netutil"
...
func (pt *PeerTable) GetAllPeers(skipRametronenterprise bool) *([]*Peer) {
	pt.mutex.Lock()
	defer pt.mutex.Unlock()
	...
	for _, p := range pt.peers {
		if skipRametronenterprise && p.NodeType() == common.NodeTypeRametronenterprise {
			continue
		}
		//ret[i] = p
		ret = append(ret, p)
...
func (pt *PeerTable) GetSelection(skipRametronenterprise bool) (peerIDAddrs []PeerIDAddress) {
	pt.mutex.Lock()
	defer pt.mutex.Unlock()
	...
	for _, peer := range peers {
		if skipRametronenterprise && peer.NodeType() == common.NodeTypeRametronenterprise {
			continue
		}
...
func (pt *PeerTable) GetTotalNumPeers(skipRametronenterprise bool) uint {
	pt.mutex.Lock()
	defer pt.mutex.Unlock()

	var numPeers uint
	if !skipRametronenterprise {
		numPeers = uint(len(pt.peers))
	} else {
		for _, peer := range pt.peers {
			if peer.NodeType() == common.NodeTypeRametronenterprise {
				continue
			}
			numPeers = numPeers + 1
		}
