...
import (
	...
	"github.com/pandoprojects/pando/dispatcher"
...
type ConsensusEngine struct {
	...
	rametronenterprise    *RametronenterpriseEngine
...
func NewConsensusEngine(privateKey *crypto.PrivateKey, db store.Store, chain *blockchain.Chain, dispatcher *dispatcher.Dispatcher, validatorManager core.ValidatorManager) *ConsensusEngine {
	...
	e.rametronenterprise = NewRametronenterpriseEngine(e, blsKey)
...
func (e *ConsensusEngine) Start(ctx context.Context) {
	...
	e.rametronenterprise.Start(e.ctx)
	...
	Epoch:
		for {
			...
			case <-e.guardianTimer.C:
				...
				rametronenterprisev := e.rametronenterprise.GetVoteToBroadcast()

				if rametronenterprisev != nil {
					e.rametronenterprise.logger.WithFields(log.Fields{"vote": rametronenterprisev}).Debug("Broadcasting aggregated rametronenterprise vote")
					e.broadcastAggregatedRametronenterpriseVotes(rametronenterprisev)
				}
				e.rametronenterprise.StartNewRound()
			}
		}
	}
}
...
func (e *ConsensusEngine) processMessage(msg interface{}) (endEpoch bool) {
	...
	case *core.RametronenterpriseVote:
		// e.logger.WithFields(log.Fields{"rametronenterprise vote": m}).Debug("Received rametronenterprise vote")
		e.handleRametronenterpriseVote(m)
	case *core.AggregatedRametronenterpriseVotes:
		// e.logger.WithFields(log.Fields{"aggregated rametronenterprise vote": m}).Debug("Received agggregated rametronenterprise vote")
		e.handleAggregatedRametronenterpriseVote(m)
	default:
		// Should not happen.
...
func (e *ConsensusEngine) validateBlock(block *core.Block, parent *core.ExtendedBlock) result.Result {
	...
	if block.GuardianVotes != nil && block.Height >= common.HeightEnablePando1 && common.IsCheckPointHeight(block.Height) {
	...
	// Validate Rametronenterprise Votes.
	// We allow checkpoint blocks to have nil rametronenterprise votes.
	if block.RametronenterpriseVotes != nil && block.Height >= common.HeightEnablePando2 && common.IsCheckPointHeight(block.Height) {
		// Voted block must exist.
		padding := uint64(20)
		if e.chain.Root().Height+padding*uint64(common.CheckpointInterval) < block.Height {
			lastCheckpoint, err := e.chain.FindBlock(block.RametronenterpriseVotes.Block)
			if err != nil {
				e.logger.WithFields(log.Fields{
					"block.Hash":               block.Hash().Hex(),
					"block.Height":             block.Height,
					"block.RametronenterpriseVotes": block.RametronenterpriseVotes.String(),
					"error":                    err.Error(),
				}).Warn("Rametronenterprise votes refers to non-existing block")
				return result.Error("Block in rametronenterprise votes cannot be found")
			}
			...
			if !e.chain.IsDescendant(lastCheckpoint.Hash(), block.Hash()) {
				...
				return result.Error("Block is not descendant of checkpoint in rametronenterprise votes")
			}
		}

		// Rametronenterprise votes must be valid.
		rametronenterprisep, err := e.ledger.GetRametronenterprisePoolOfLastCheckpoint(block.RametronenterpriseVotes.Block)
		if err != nil {
			e.logger.WithFields(log.Fields{
				"block.Hash":               block.Hash().Hex(),
				"block.Height":             block.Height,
				"block.RametronenterpriseVotes": block.RametronenterpriseVotes.String(),
				"error":                    err.Error(),
			}).Warn("Failed to load rametronenterprise pool")
			return result.Error("Failed to load rametronenterprise pool")
		}
		if res := block.RametronenterpriseVotes.Validate(rametronenterprisep); res.IsError() {
			e.logger.WithFields(log.Fields{
				"block.Hash":               block.Hash().Hex(),
				"block.Height":             block.Height,
				"block.RametronenterpriseVotes": block.RametronenterpriseVotes.String(),
				"error":                    res.String(),
			}).Warn("Failed to validate rametronenterprise votes attached to the block")
			return result.Error("Rametronenterprise votes are not valid")
		}
	} else {
		if block.RametronenterpriseVotes != nil {
			e.logger.WithFields(log.Fields{
				"block.Epoch":              block.Epoch,
				"block.proposer":           block.Proposer.Hex(),
				"block.Hash":               block.Hash().Hex(),
				"block.Height":             block.Height,
				"block.RametronenterpriseVotes": block.RametronenterpriseVotes.String(),
			}).Warn("Rametronenterprise votes in non-checkpoint block")
			return result.Error("Non-checkpoint block should not have rametronenterprise votes")
		}
	}
...
func (e *ConsensusEngine) handleRametronenterpriseVote(v *core.RametronenterpriseVote) {
	e.rametronenterprise.HandleVote(v)
}

func (e *ConsensusEngine) handleAggregatedRametronenterpriseVote(v *core.AggregatedRametronenterpriseVotes) {
	e.rametronenterprise.HandleAggregatedVote(v)
}

func (e *ConsensusEngine) broadcastAggregatedRametronenterpriseVotes(vote *core.AggregatedRametronenterpriseVotes) {
	payload, err := rlp.EncodeToBytes(vote)
	if err != nil {
		e.logger.WithFields(log.Fields{"rametronenterprise vote": vote}).Error("Failed to encode vote")
		return
	}
	voteMsg := dispatcher.DataResponse{
		ChannelID: common.ChannelIDAggregatedRametronenterpriseVotes,
		Payload:   payload,
	}
	e.dispatcher.SendData([]string{}, voteMsg)
}
...
func (e *ConsensusEngine) finalizeBlock(block *core.ExtendedBlock) error {
	...
	// Guardians and Rametronenterprises to vote for checkpoint blocks.
	if common.IsCheckPointHeight(block.Height) {
		e.guardian.StartNewBlock(block.Hash())
		e.rametronenterprise.StartNewBlock(block.Hash())
		e.resetGuardianTimer()
	}
...
func (e *ConsensusEngine) createProposal(shouldIncludeValidatorUpdateTxs bool) (core.Proposal, error) {
	...
	// Add guardian votes.
	if block.Height >= common.HeightEnablePando1 && common.IsCheckPointHeight(block.Height) {
		block.GuardianVotes = e.guardian.GetBestVote()
	}

	// Add rametronenterprise votes.
	if block.Height >= common.HeightEnablePando2 && common.IsCheckPointHeight(block.Height) {
		block.RametronenterpriseVotes = e.rametronenterprise.GetBestVote()
	}
